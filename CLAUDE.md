...
### Memory 模块关键文件
- `backend/app/memory/memory/main.py` - Memory 引擎核心 (add/search/get/update/delete)
- `backend/app/memory/memory/graph_memory.py` - Neo4j 图记忆 (纯 Cypher 余弦相似度)
- `backend/app/memory/services/memory_worker.py` - 后台 Worker (fact extraction + graph build)
- `backend/app/memory/services/memory_client.py` - Memory 客户端工厂
- `backend/app/api/memory_compat.py` - 兼容路由 (v1/v2 映射到 /memory/*)
- `backend/app/api/memory_ailearn.py` - AI 学习 API 路由
...
backend/app/memory/
├── memory/              # 核心引擎
│   ├── main.py          # Memory 类 (add/search/get/update/delete)
│   ├── graph_memory.py  # Neo4j 图记忆
│   ├── telemetry.py     # 遥测 (已禁用)
│   └── setup.py         # 初始化
├── configs/             # 配置定义
├── embeddings/          # 向量嵌入 providers
├── graphs/              # 图数据库 providers
├── llms/                # LLM providers
├── vector_stores/       # 向量存储 providers (Qdrant 为主)
├── services/            # 业务服务
│   ├── memory_worker.py # 后台 Worker
│   └── memory_client.py # 客户端工厂
├── ailearn/             # AI 自主学习引擎
├── observation/         # 观察层
├── learning/            # 学习层
├── instincts/           # 直觉层
