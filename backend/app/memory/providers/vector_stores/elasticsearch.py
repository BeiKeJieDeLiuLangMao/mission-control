...
from app.memory.configs.vector_stores.elasticsearch import ElasticsearchConfig
...
class ElasticsearchDB(VectorStoreBase):
    def __init__(self, **kwargs):
        ...
        if config.cloud_id:
            ...
            self.client = Elasticsearch(
                ...
                basic_auth=(
                    (config.user, config.password) if (config.user and config.password) else None
                ),
                verify_certs=config.verify_certs,
                headers=config.headers or {},
            )
    ...
    def create_index(self) -> None:
        ...
        index_settings = {
            "settings": {
                "index": {"number_of_replicas": 1, "number_of_shards": 5, "refresh_interval": "1s"}
            },
            "mappings": {
                "properties": {
                    "text": {"type": "text"},
