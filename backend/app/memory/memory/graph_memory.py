...
from app.memory.memory.utils import format_entities, sanitize_relationship_for_cypher
...
from app.memory.utils.factory import EmbedderFactory, LlmFactory
...
class MemoryGraph:
    ...
    def _search_graph_db(self, node_list, filters, limit=100):
        ...
        for node in node_list:
            ...
            cypher_query = f"""
            MATCH (n {self.node_label} {{{node_props_str}}})
            WHERE n.embedding IS NOT NULL AND size(n.embedding) = size($n_embedding)
            WITH n,
                 reduce(dot = 0.0, i IN range(0, size(n.embedding)-1) | dot + n.embedding[i] * $n_embedding[i]) AS dotProduct,
                 sqrt(reduce(s = 0.0, x IN n.embedding | s + x*x)) AS normA,
                 sqrt(reduce(s = 0.0, x IN $n_embedding | s + x*x)) AS normB
            WITH n, round(2 * (dotProduct / (normA * normB)) - 1, 4) AS similarity
    ...
    def _add_entities(self, to_be_added, filters, entity_type_map):
        ...
        agent_id = filters.get("agent_id", None)
        run_id = filters.get("run_id", None)
        turn_id = filters.get("turn_id", None)  # Extract turn_id from filters
        results = []
        for item in to_be_added:
            ...
            if not destination_node_search_result and source_node_search_result:
                ...
                if turn_id:  # Add turn_id to node properties
                    merge_props.append("turn_id: $turn_id")
                ...
                if run_id:
                    params["run_id"] = run_id
                if turn_id:  # Add turn_id to params
                ...
                if run_id:
                    source_props.append("run_id: $run_id")
                    dest_props.append("run_id: $run_id")
                if turn_id:  # Add turn_id to both source and destination
                    source_props.append("turn_id: $turn_id")
                    dest_props.append("turn_id: $turn_id")
                source_props_str = ", ".join(source_props)
                dest_props_str = ", ".join(dest_props)
    ...
    def _search_source_node(self, source_embedding, filters, threshold=0.9):
        ...
        cypher = f"""
            ...
            WITH source_candidate,
                 reduce(dot = 0.0, i IN range(0, size(source_candidate.embedding)-1) | dot + source_candidate.embedding[i] * $source_embedding[i]) AS dotProduct,
                 sqrt(reduce(s = 0.0, x IN source_candidate.embedding | s + x*x)) AS normA,
                 sqrt(reduce(s = 0.0, x IN $source_embedding | s + x*x)) AS normB
            WITH source_candidate, round(2 * (dotProduct / (normA * normB)) - 1, 4) AS source_similarity
    ...
    def _search_destination_node(self, destination_embedding, filters, threshold=0.9):
        ...
        cypher = f"""
            ...
            WITH destination_candidate,
                 reduce(dot = 0.0, i IN range(0, size(destination_candidate.embedding)-1) | dot + destination_candidate.embedding[i] * $destination_embedding[i]) AS dotProduct,
                 sqrt(reduce(s = 0.0, x IN destination_candidate.embedding | s + x*x)) AS normA,
                 sqrt(reduce(s = 0.0, x IN $destination_embedding | s + x*x)) AS normB
            WITH destination_candidate, round(2 * (dotProduct / (normA * normB)) - 1, 4) AS destination_similarity
