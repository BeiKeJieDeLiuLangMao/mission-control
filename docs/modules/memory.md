...
├── client/                           #   REST 客户端 SDK
└── services/                         #   后台服务
    ├── recall_orchestrator.py        #   智能并发召回 (QueryAnalyzer + 三阶段编排)
...
│    per-turn: detect_type → Memory Engine (fact/correction/procedure/summary/graph) │
...
┌─────────────────────────────────────────────────────────────────┐
│  插件层 (Claude Code hooks / OpenClaw TypeScript 插件)           │
│  HTTP → POST http://localhost:8000/api/v2/recall               │
│  {user_id, query, agent_id?, context_budget_tokens?}           │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ↓
┌─────────────────────────────────────────────────────────────────┐
│  RecallOrchestrator (FastAPI :8000)                              │
│  Stage 1: QueryAnalyzer (intent/entities/complexity, 纯 regex)  │
│  Stage 2: asyncio.gather (Qdrant + Neo4j + corrections)        │
│  Stage 3: Merge + Rank (corrections > vectors > graph) + Format│
└────────────────────────────┬────────────────────────────────────┘
                             │
         ┌───────────────────┴───────────────────┐
         ↓                                       ↓
...
Fallback: 召回失败时适配器自动降级到 `GET /api/v2/memories/search` (纯向量搜索)。
...
| 消费者 | 路由 | 用途 | 代码 |
|--------|------|------|------|
| Claude Code hooks | `POST /api/v2/recall` | **智能并发召回** (优先) | `api/memory/intelligent_recall.py` |
| Claude Code hooks | `POST /api/v2/turns/` | 存储对话轮次 | `api/memory/adapter_turns.py` |
| Claude Code hooks | `GET /api/v2/memories/search` | 召回记忆 (fallback) | `api/memory/adapter_compat.py` |
| OpenClaw 插件 | `POST /api/v2/recall` | **智能并发召回** (优先) | `api/memory/intelligent_recall.py` |
| OpenClaw 插件 | `POST /api/v1/turns/` | 存储对话轮次 | `api/memory/adapter_compat.py` |
| OpenClaw 插件 | `GET /api/v2/memories/search` | 召回记忆 (fallback) | `api/memory/adapter_compat.py` |
...
| 前端 AILearn 页 | `GET /api/v1/ailearn/patterns` | 检测到的模式 | `api/memory/frontend_ailearn.py` |
| 前端 AILearn 页 | `GET /api/v1/ailearn/skills` | 提取的技能 | `api/memory/frontend_ailearn.py` |
| 前端 AILearn 页 | `GET /api/v1/ailearn/amendments` | 修订提案 | `api/memory/frontend_ailearn.py` |
| 前端/适配器 | `GET /api/v1/tasks/segments` | 任务分段列表 | `api/memory/task_segments.py` |
| 前端/适配器 | `GET /api/v1/tasks/segments/stats` | 任务分段统计 | `api/memory/task_segments.py` |
| 前端/适配器 | `GET /api/v1/tasks/segments/{id}` | 任务分段详情 | `api/memory/task_segments.py` |
| 适配器 | `GET /api/v2/tasks/` | 任务分段列表 (v2) | `api/memory/adapter_compat.py` |
| 内部路由 | `/memory/turns/`, `/memory/memories/` | 原始 CRUD | `api/memory/internal_crud.py` |
| 内部路由 | `/memory/tasks/` | 任务分段 CRUD | `api/memory/task_segments.py` |
...
### 列表视图
- 记忆卡片: 内容 + source 徽章 + memory_type 徽章 (事实/摘要/纠正/流程/任务事实) + agent + 时间
- 筛选: 按来源 (OpenClaw/Claude Code) + 按 Agent
- 搜索: 向量语义搜索 (Qdrant 优先) + SQL 关键词备选 (`api/memory/internal_crud.py:search_memories`)
...
| `memory/services/recall_orchestrator.py` | 智能并发召回 (QueryAnalyzer + RecallOrchestrator) |
...
| `api/memory/intelligent_recall.py` | POST /api/v2/recall 端点 |
...
| `api/memory/task_segments.py` | Task Segment API (list/detail/stats) |
...
| `RECALL_VECTOR_TIMEOUT_MS` | `15000` | Recall 向量搜索超时 (ms) |
| `RECALL_GRAPH_TIMEOUT_MS` | `5000` | Recall 图搜索超时 (ms) |
| `RECALL_DEFAULT_BUDGET_TOKENS` | `2000` | Recall 默认 context token 预算 |
...
API: `POST /api/v1/ailearn/start`、`GET /api/v1/ailearn/status`、`GET /api/v1/ailearn/patterns`、`GET /api/v1/ailearn/skills`、`GET /api/v1/ailearn/amendments`

### 学习闭环

Worker 在 `process_fact_extraction()` 中通过规则匹配自动检测纠正和过程类型消息：

```
用户纠正 ("不对，应该是 X") → Worker detect_memory_type() → memory_type="correction"
                                                          ↓
                                                   Qdrant 存储 + PG 记录
                                                          ↓
                                    下次 recall 时 correction 优先级最高 (100) → Agent 不再犯同样错误
```

支持的 memory_type: `fact` (默认) / `correction` (纠正) / `procedure` (流程) / `summary` (摘要) / `task_fact` (任务事实)
...
# 验证记忆 (智能召回)
curl -X POST http://localhost:8000/api/v2/recall \
  -H "Content-Type: application/json" \
  -d '{"user_id":"test","query":"Python相关","context_budget_tokens":2000}'
